...
export const UI = {
  ...
  askAiHint: {
    ja: "作りたいものを書いて「AI で作る」を押すと、AI タブのキーで設計がキャンバスに描かれます。スクリーンショットを添付すれば、既存のアプリを再現することもできます。代わりに指示をコピーして Claude Code などの AI エージェントに渡すこともできます。返ってきたリンクを開くか、JSON をファイルに保存して「プロジェクトを開く」から読み込みます。",
    en: "Describe what you want and press Draft with AI: the model from the AI tab draws the design on the canvas. Attach screenshots to have it replicate an existing app. Or copy the instruction for an AI agent such as Claude Code; open the link it replies with, or save its JSON and load it with Open project.",
    zh: "写下想做的东西并按“让 AI 生成”，AI 标签页里的模型会把设计画到画布上；附上截图还能让 AI 复刻现有的应用。也可以复制指令交给 Claude Code 等 AI 代理：打开它回复的链接，或把 JSON 保存为文件后用“打开项目”读取。",
  },
  ...
  askAiAttach: { ja: "画像を追加", en: "Add image", zh: "添加图片" },
  askAiAttachTitle: { ja: "既存アプリのスクリーンショットを添付すると AI が再現します（最大 4 枚、貼り付けも可）", en: "Attach screenshots of an existing app for the model to replicate (up to 4; pasting works too)", zh: "附上现有应用的截图让 AI 复刻（最多 4 张，也可直接粘贴）" },
  askAiRemoveImage: { ja: "画像を削除", en: "Remove image", zh: "移除图片" },
  askAiImagesHint: { ja: "添付したスクリーンショットを AI が再現します。テキストは補足として使われます。", en: "The model replicates the attached screenshots; the text is read as your notes.", zh: "AI 会复刻附带的截图，文字作为补充说明。" },
  askAiNoVision: { ja: "選択中のプロバイダーは画像を読めない可能性があります", en: "The selected provider may not read images", zh: "当前提供商可能不支持看图" },
...
export const KO: Record<UIKey, string> = {
  ...
  askAi: "AI에게 맡기기",
  askAiHint: "만들고 싶은 것을 적고 'AI로 만들기'를 누르면 AI 탭의 키로 설계가 캔버스에 그려집니다. 스크린샷을 첨부하면 기존 앱을 재현할 수도 있습니다. 대신 지시를 복사해 Claude Code 같은 AI 에이전트에 넘길 수도 있습니다. 답한 링크를 열거나 JSON을 파일로 저장해 '프로젝트 열기'로 불러오세요.",
  askAiGenerate: "AI로 만들기", askAiGenerating: "만드는 중…",
  askAiAttach: "이미지 추가", askAiAttachTitle: "기존 앱의 스크린샷을 첨부하면 AI가 재현합니다(최대 4장, 붙여넣기도 가능)", askAiRemoveImage: "이미지 삭제",
  askAiImagesHint: "첨부한 스크린샷을 AI가 재현합니다. 텍스트는 보충 설명으로 쓰입니다.", askAiNoVision: "선택한 제공자는 이미지를 읽지 못할 수 있습니다",
